...
#include <winreg.h>
  ...
  /* Linker complains about undefined reference to rpl_malloc(). */
  #undef malloc
#endif
#else
/* Linux */
...
// counter of active requests running on context
int dnssec_glob_val_ctx_use_count = 0;
int dnssec_glob_val_ctx_flush_pending = 0;
...
{
	if (!dnssec_glob_val_ctx_use_count) {
		/* Has separate initialisation procedure. */
    ...
    const char *optdnssrv, const char *ipbrowser, char **ipvalidator)
{
	...
	// release active request counter
	dnssec_glob_val_ctx_use_count--;
	if (!dnssec_glob_val_ctx_use_count && dnssec_glob_val_ctx_flush_pending) {
		dnssec_glob_val_ctx_flush_pending = 0; // kill the pending flag
...
{	
	if (!dnssec_glob_val_ctx_use_count) {
		...
		dnssec_glob_val_ctx_flush_pending = 1;
	}
